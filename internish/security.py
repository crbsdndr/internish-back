...
def make_refresh_token(sub: str, extra: Dict[str, Any] | None = None) -> str:
...
def require_auth(token: str = Depends(oauth2_scheme)) -> dict:
    ...
    if payload.get("type") != "access":
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail={"status": False, "message": "Wrong token type"},
        )
...
def require_role(payload: dict = Depends(require_auth)) -> dict:
    if payload.get("role") == "student":
        raise HTTPException(
            ...
            detail={"status": False, "message": "Your role cannot access this resource"},
        )
    return payload
